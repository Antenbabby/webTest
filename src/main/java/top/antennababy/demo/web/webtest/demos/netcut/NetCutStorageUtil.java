 ...
 * 用于存储一些简单的数据
 * key-value形式
 * <a href="https://netcut.cn/">https://netcut.cn/</a>
 */
public class NetCutStorageUtil {
    //apiurl
...
//        timedCache.schedulePrune(5);
    ...
    private static void save(JSONObject data) {
        ...
        map.put("note_name", NOTE_NAME);
        map.put("note_pwd", PSW);
        map.put("note_content", Base64.encode(SecureUtil.aes(CONTENT_PSW.getBytes(StandardCharsets.UTF_8)).encrypt(JSONUtil.toJsonStr(data).getBytes(StandardCharsets.UTF_8))));
        map.put("note_id", storgeInfo.get("note_id"));
        map.put("note_token", storgeInfo.get("note_token"));
        map.put("expire_time", storgeInfo.get("expire_time"));
        String post = HttpUtil.post(URL + "/netcut/note/save/", map);
        JSONObject ret = JSONUtil.parseObj(post);
        //成功
        if (ObjUtil.equals( ret.get("status"), 1)) {
            storgeInfo.set("note_id",ret.getJSONObject("data").getStr("note_id"));
            storgeInfo.set("note_token",ret.getJSONObject("data").getStr("note_token"));
    ...
    public static JSONObject getAll() {
        ...
        String post = HttpUtil.post(URL + "/netcut/note/info/", Dict.of("note_name", NOTE_NAME,"note_pwd",PSW));
        JSONObject ret = JSONUtil.parseObj(post);
        //成功
        if (ObjUtil.equals( ret.get("status"), 1)) {
            storgeInfo=ret.getJSONObject("data");
            JSONObject retObj = JSONUtil.parseObj(Opt.ofBlankAble(decryptStr(ret.getJSONObject("data").getStr("note_content"))).orElse("{}"));
